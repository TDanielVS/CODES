...
model.add(Cropping2D(cropping=((115, 0), (0, 0))))  # 65, 25
...
saveBestModel = callbacks.ModelCheckpoint(filepath=path + 'model.h5', monitor='val_loss', verbose=1,
                                          save_best_only=True, mode='auto')
history_object = model.fit_generator(train_generator, samples_per_epoch=len(train_samples) * 2,
                                     validation_data=validation_generator, nb_val_samples=len(validation_samples)
                                     , nb_epoch=5, callbacks=[earlyStopping, saveBestModel])
